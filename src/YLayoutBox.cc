...
void YLayoutBox::move_up(YWidget * child)
{
    yuiMilestone() << "up: " << child << std::endl;
    auto mgr = childrenManager();

    if (child == mgr->firstChild())
    {
        yuiMilestone() << "already the first child" << std::endl;
        return;
    }

    auto i = find( mgr->begin(), mgr->end(), child );
    if (i != mgr->end())
    {
        auto i2 = i--;
        std::swap(*i2, *i);
...
void YLayoutBox::move_down(YWidget * child)
{
    yuiMilestone() << "down: " << child << std::endl;
    auto mgr = childrenManager();

    if (child == mgr->lastChild())
    {
        yuiMilestone() << "already the last child" << std::endl;
        return;
    }
